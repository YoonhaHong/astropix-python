...
def injection_each_col(args, astro, col):


    print(f"Start injecting for COL{col} ")
    ...
    for rowi in range(args.rowrange[0], args.rowrange[1], 1):
        astro.asic.set_inj_row(rowi, True)
        astro.enable_pixel(col=col, row=rowi)
    
def injection_each_row(args, astro, row):


    print(f"Start injecting for ROW{row} ")
    ...
    for coli in range(args.colrange[0], args.colrange[1], 1):
        astro.asic.set_inj_col(coli, True)
        astro.enable_pixel(col=coli, row=row)
...
if __name__ == "__main__":
    ...
    #    injection_each_col(args, astro, col)
    #    bitpath = os.path.join(args.outdir, f"col{col}.log")
    for row in range(args.rowrange[0], args.rowrange[1], 1):
        injection_each_row(args, astro, row)
        bitpath = os.path.join(args.outdir, f"row{row}.log")
