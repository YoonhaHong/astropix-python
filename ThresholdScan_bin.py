...
#Init 
def main(args, row, col, injv, threshold, fpgaCon:bool=True, fpgaDiscon:bool=True):
    ...
    strPix = f"r{row}c{col}_inj{injv}V_thr{threshold}V_"
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i', '--vinj', action='store', default=300, type=float,
                    help =  'Injection voltage in mV. Default: 300')
    
    parser.add_argument('-V', '--Voltage', action='store', default=[0, 600], type=int, nargs=2,
                    help =  'Range of voltages to scan through in mV. Default: 0 600')

    parser.add_argument('-p', '--precision', action='store', default=10, type=int,
                    help =  'Precision of voltage scan in mV. Default: 10')
    ...
    for r in tqdm(range(args.rowrange[0],args.rowrange[1]+1,1), 
                  desc='Row     ', position=0, leave=False):
        for c in tqdm(range(args.colrange[0],args.colrange[1]+1,1), 
                  desc='Col     ', position=1, leave=False):
            step = 1
    
            v_start = args.Voltage[0]
            v_stop = args.Voltage[1]

            start_temp = v_start
            stop_temp = v_stop

            measure_at_zero = True
            while (stop_temp - start_temp) >= args.precision:

                if measure_at_zero:
                    v_bin = v_stop  # Additional point at max
                    readout = main(args, r, c, injv=float(args.vinj), threshold=v_bin, fpgaDiscon=False)
                else:
                    v_bin = np.round((start_temp + stop_temp) / 2)
                    readout = main(args, r, c, injv=float(args.vinj), threshold=v_bin, fpgaCon=False, fpgaDiscon=False)
                

                
                if not measure_at_zero:
                    if readout < args.maxruns:
                        start_temp = v_bin
                    else:
                        stop_temp = v_bin
            ...
            main(args, r, c, injv=float(args.vinj), threshold=v_bin, fpgaCon=False)
